function filelist_formatted = DAG_get_filelist_from_xls(Aiminput,Inputsequal,Inputsrange,Inputslist, datapath, xlspath) %Inputslist
...
% Last updated 20150824
%%

filelist_formatted=[];

[num,masterstring,mastercomplete] = xlsread(xlspath,'Mastertable');
%masterstring=mastercomplete;
%masternum=[NaN(1,size(num,2));num];

idx_Session=DAG_find_column_index(masterstring,'Session');
if isempty(idx_Session)
idx_Session=DAG_find_column_index(masterstring,'Date');
    
end
idx_Run=DAG_find_column_index(masterstring,'Run');
idx_Task_Aim=DAG_find_column_index(masterstring,'Aim');

log_sel_aim_idx=ones(1,size(mastercomplete,1));
log_sel_equ_idx=ones(1,size(mastercomplete,1));
log_sel_lower_ran_idx=ones(1,size(mastercomplete,1));
log_sel_upper_ran_idx=ones(1,size(mastercomplete,1));
log_sel_list_idx=ones(1,size(mastercomplete,1));
...
for equalidx=1:size(Inputsequal,1)
   ...
   log_sel_equ_idx(equalidx,:)=[NaN mastercomplete{2:end,idx.(Inputsequal{equalidx,1})}]==Inputsequal{equalidx,2};
   end
end

for ranidx=1:size(Inputsrange,1)
   idx.(Inputsrange{ranidx,1})=DAG_find_column_index(masterstring,Inputsrange{ranidx,1});
   log_sel_lower_ran_idx(ranidx,:)=[NaN mastercomplete{2:end,idx.(Inputsrange{ranidx,1})}]>=Inputsrange{ranidx,2};
   log_sel_upper_ran_idx(ranidx,:)=[NaN mastercomplete{2:end,idx.(Inputsrange{ranidx,1})}]<=Inputsrange{ranidx,3};
end
...
new_idx=1;
 for master_idx = 1: size(mastercomplete,1) %length(raw(:,1))

     if  Sel_all(master_idx)    
        cell_path{new_idx}      = datapath;
        cell_session{new_idx}   = mastercomplete{master_idx,idx_Session};
        cell_run{new_idx}       = mastercomplete{master_idx,idx_Run};
